...
public:
	...
	~VulkanExample()
	{
		vkDestroyQueryPool(this->device, this->timestampQueryPool, nullptr);
		this->timestampQueryPool = VK_NULL_HANDLE;
	...
	{
		vkResetCommandBuffer(drawCmdBuffers[currentBuffer], VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			vkCmdWriteTimestamp(drawCmdBuffers[i], VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, this->timestampQueryPool, i * 2 + 0);
			vkCmdWriteTimestamp(drawCmdBuffers[i], VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, this->timestampQueryPool, i * 2 + 1);
			...
			float const Interpolate = std::sin(timer);

			// Submit via push constant (rather than a UBO)
			vkCmdPushConstants(drawCmdBuffers[i], pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(Interpolate), &Interpolate);
...
/*
		...
		// Submit via push constant (rather than a UBO)
		vkCmdPushConstants(drawCmdBuffers[currentBuffer], pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(Interpolate), &Interpolate);
...
*/
		...
		// Wait for the fence to signal that command buffer has finished executing
		VK_CHECK_RESULT(vkWaitForFences(device, 1, &waitFences[currentBuffer], VK_TRUE, DEFAULT_FENCE_TIMEOUT));
	...
	void prepare()
	{
		...
		VK_CHECK_RESULT(vkCreateQueryPool(this->device, &ci, nullptr, &this->timestampQueryPool));
